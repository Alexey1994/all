...
Number16 sound_mixer_data[SOUND_BUFFER_LENGTH / 2];
...
{
	Integer_Number32 sound_mixer_data_temp[SOUND_BUFFER_LENGTH / 2];
	Integer_Number16 current_sound_mixer_data[SOUND_BUFFER_LENGTH / 2];
	Number   i;
	Number   j;

	clear_bytes(sound_mixer_data_temp, SOUND_BUFFER_LENGTH * 2);
	...
	for(i = 0; i < number_of_mixed_readers; ++i)
	{
		...
		for(j = 0; j < SOUND_BUFFER_LENGTH / 2; ++j)
...
void initialize_sound_mixer()
{
	number_of_mixed_readers = 0;
	initialize_sound_device(0, &mix_sound);
}
