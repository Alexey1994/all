...
{
	if(reader->destroy_source)
		reader->destroy_source(reader->source);

	deinitialize_buffer(&reader->buffer);
}


void next_reader_byte(Reader* reader)
{
...
static Integer_Number read_bytes_from_buffered_reader(Buffered_Reader_Source* source, Byte* bytes, Number bytes_length)
{
	*bytes = get_reader_byte(source->reader);
	next_reader_byte(source->reader);
...
Number read_bytes_from_null_terminated_bytes_source(Byte** null_terminated_bytes_source, Byte* bytes, Number number_of_bytes)
{
	Number bytes_readed;
...
Boolean end_of_null_terminated_bytes_source_data(Byte** null_terminated_bytes_source)
{
...
void initialize_null_terminated_bytes_reader(Reader* reader, Byte* null_terminated_bytes)
{
	...
	*null_terminated_bytes_source = null_terminated_bytes;
	...
	reader->destroy_source = &free_memory;
}
