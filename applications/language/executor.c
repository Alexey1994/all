void execute(
	...
	void    (*enter_expression)(Byte* execution_context, Expression* expression)
)
{
	...
	N_32 expressions_length = expressions->length / sizeof(Expression);
	while(expression_index < expressions_length)
	{
		Expression expression;
		get_buffer_bytes(expressions, expression_index * sizeof(Expression), &expression, sizeof(Expression));
		
		if(enter_expression)
		{
			enter_expression(execution_context, &expression);
			set_buffer_bytes(expressions, expression_index * sizeof(Expression), &expression, sizeof(Expression));
		}
